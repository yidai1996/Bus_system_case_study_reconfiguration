...
for (i,transformer) in ref[:transformer]
    ...
    if configuration == PMD.WYE
		...
		tm = [tm_fixed[idx] ? tm_set[idx] : tap[i][idx] for (idx,(fc,tc)) in enumerate(zip(f_connections,t_connections))]
		...
		tmsqr = [
			tm_fixed[i] ? tm[i]^2 : JuMP.@variable(
				model,
				base_name="0_tmsqr_$(i)_$(f_connections[i])",
				start=(JuMP.start_value(tm[i]) !== nothing ? JuMP.start_value(tm[i])^2 : 1.0),
				lower_bound=JuMP.has_lower_bound(tm[i]) ? JuMP.lower_bound(tm[i])^2 : 0.9^2,
				upper_bound=JuMP.has_upper_bound(tm[i]) ? JuMP.upper_bound(tm[i])^2 : 1.1^2
			) for i in 1:length(tm)
		]
	    ...
	    for (idx, (fc, tc)) in enumerate(zip(f_connections, t_connections))
	        if tm_fixed[idx]
				...
				tmsqr_w_to = JuMP.@variable(model, base_name="0_tmsqr_w_to_$(i)_$(t_bus)_$(tc)")
